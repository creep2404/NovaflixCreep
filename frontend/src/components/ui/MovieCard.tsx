...
export const MovieCard: React.FC<MovieCardProps> = ({ movie, onClick, orientation = 'portrait' }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
        <h3 className="font-headline font-semibold text-lg line-clamp-1 mb-1">{movie.title}</h3>
        
        <div className="flex items-center gap-2 text-xs text-on-surface-variant mb-3">
          {movie.matchPercentage && <span className="text-green-400 font-medium">{movie.matchPercentage}% Match</span>}
          <span>{movie.year}</span>
          <span className="px-1.5 py-0.5 border border-white/20 rounded text-[10px]">{movie.rating}</span>
          <span>{movie.duration}</span>
        </div>
        
        <div className="flex gap-2">
          <button className="flex-1 flex items-center justify-center gap-1 bg-white text-black py-1.5 rounded-md font-medium text-sm hover:bg-white/90 transition-colors">
            <Play size={14} className="fill-current" /> Play
          </button>
          <button className="p-1.5 bg-white/20 hover:bg-white/30 rounded-md backdrop-blur-sm transition-colors text-white">
            <Plus size={18} />
          </button>
        </div>
      </div>
